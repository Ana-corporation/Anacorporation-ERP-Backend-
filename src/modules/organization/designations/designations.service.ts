...
export class DesignationsService {
  ...
  async create(companyId: string, dto: CreateDesignationDto, actorId: string) {
    const { code, autoGenerated } = await resolveEntityCode({
      explicitCode: dto.designationCode,
      name: dto.name,
      fallback: 'DESIG',
    ...
    if (!autoGenerated && (await this.repository.findByCode(companyId, code))) {
      throw new ConflictException(
        'Designation code already exists in this company',
    ...
    await this.auditService.log({
      ...
      newValue: { designationCode: code, name: designation.name, autoGenerated },
    });
